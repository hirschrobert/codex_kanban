...
class ProjectCliTest(ProjectCliCase):
    def test_project_prompt_instructs_split_multi_intent_intake(self) -> None:
        ...
        self.assertIn("delegation under the main Codex agent's control", output)
        self.assertIn("optional task-specific offers", output)
        self.assertIn("Codex built-in", output)
        self.assertIn("does not require delegation", output)
        self.assertIn("exact custom-agent type", output)
        self.assertIn("Codex subagents People role", output)
